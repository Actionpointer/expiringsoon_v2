...
$sql = "INSERT INTO products (userid, cat_id, subcat_id, product, price, stock, expiry, info, tags, status, photo, uploaded) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
$conn->prepare($sql)->execute([$uqr['id'], $_POST['cat_id'], $_POST['subcat_id'], ucwords($product), $_POST['price'], $_POST['stock'], $_POST['expiry'], $info2, 'None', 'Listed', $path.$actual_image_name, $now]);
header("Location: addproduct.php?saved");
}
}
}
}
...
<head>
    ...
    <title>Add Product | Expiring Soon</title>
    <link
      rel="icon"
      type="image/png"
...
var loadFile = function(event) {
  ...
  <body>
    ...
    echo'
    <div class="notify">
    <p style="color:#fff">Product Uploaded Successfully</p>
    </div>
    ';
    }
    if(isset($_GET['fail'])){
    echo'
    <div class="notify">
    <p style="color:#fff">Please upload a JPG or PNG image only</p>
    </div>
    ';
    }
    ?>
