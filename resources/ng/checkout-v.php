...
$check = "SELECT * FROM cart WHERE userid='".$uqr['id']."' AND vendor='".$_GET['v']."' AND status='Open' AND orderid!='$orderid'";
if ($result=mysqli_query($con,$check)){
if (mysqli_num_rows($result) >= 1){

$sum = "SELECT sum(total) as ttl FROM cart WHERE userid='".$uqr['id']."' AND vendor='".$_GET['v']."' AND status='Open'";
...
$con->query("UPDATE cart SET orderid='$orderid' WHERE userid='".$uqr['id']."' AND vendor='".$_GET['v']."' AND status='Open' AND orderid!='$orderid'");
...
$sum = "SELECT sum(total) as ttl FROM cart WHERE orderid='$orderid' AND vendor='".$_GET['v']."'";
$res = mysqli_query($con, $sum);
while ($row = mysqli_fetch_array($res)){
$orderttl = $row['ttl'];
}
...
if(isset($_POST['btn-wallet'])) {
  ...
  curl_setopt_array($curl, array(
    ...
    $con->query("UPDATE orders SET status='Paid', paymentmethod='Wallet', deliverystatus='Processing', trackingcode='$trackingcode' WHERE userid='".$uqr['id']."' AND orderid='$rand'");
...
if(isset($_POST['btn-cash'])) {
  ...
  $con->query("UPDATE orders SET total='$neworderttl' WHERE orderid='$orderid'");
  $con->query("UPDATE cart SET status='Closed' WHERE userid='".$uqr['id']."' AND orderid='$rand' AND vendor='".$_GET['v']."'");
  $con->query("UPDATE orders SET status='Paid', paymentmethod='Cash', deliverystatus='Processing', trackingcode='$trackingcode' WHERE userid='".$uqr['id']."' AND orderid='$rand'");
  if($oqr['recipient'] == 'None'){
  $con->query("UPDATE orders SET recipient='".ucwords($uqr['fname'].' '.$uqr['lname'])."', deliverycontact='".$uqr['phone']."', deliveryfee='".$oqr['deliveryfee']."', discount='".$oqr['discount']."' WHERE userid='".$uqr['id']."' AND orderid='$rand'");
  }
  $con->commit();
              ...
              <div class="bill-card__content">
                ...
                <div class="bill-card__body">
                  ...
                  <div class="bill-card__product">
                    <?php
                    $query = "SELECT * FROM cart WHERE userid='".$uqr['id']."' AND orderid='".$_SESSION['orderid']."' AND status='Open' AND vendor='".$_GET['v']."' ORDER BY date DESC";
                    $qq = mysqli_query($con, $query);
                    while ($value = mysqli_fetch_array($qq)){
