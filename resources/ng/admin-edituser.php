...
$usrq = mysqli_query($con, "SELECT * FROM users WHERE id='".$_GET['uid']."'");
$usrqr = mysqli_fetch_assoc($usrq);

if (isset($_POST['btn-save'])){
if(!isset($_FILES['imageUpload']) && empty($_FILES['imageUpload']['name'])) {
$con->autocommit(FALSE);
$con->query("UPDATE products SET product='".ucwords($_POST['product'])."', info='".$_POST['info']."', stock='".$_POST['stock']."', price='".$_POST['price']."', tags='".$_POST['tags']."' WHERE id='".$_POST['pid']."'");
$con->commit();
header("Location: admin-edituser.php?uid=".$_POST['pid']."&saved");
exit();
}
if(isset($_FILES['imageUpload']) && !empty($_FILES['imageUpload']['name'])) {
$path = "uploads/";
...
header("Location: admin-edituser.php?uid=".$_POST['pid']."&fail");
exit();
}
...
$con->autocommit(FALSE);
$con->query("UPDATE products SET product='".ucwords($_POST['product'])."', info='".$_POST['info']."', stock='".$_POST['stock']."', price='".$_POST['price']."', tags='".$_POST['tags']."', photo='$path$actual_image_name' WHERE id='".$_POST['pid']."'");
$con->commit();
header("Location: admin-edituser.php?uid=".$_POST['pid']."&saved");
}
}
...
<!-- Mirrored from shopery.netlify.app/main/user-dashboard.html by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 29 Dec 2021 14:28:41 GMT -->
<!-- Added by HTTrack --><meta http-equiv="content-type" content="text/html;charset=UTF-8" /><!-- /Added by HTTrack -->
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta http-equiv="X-UA-Compatible" content="ie=edge" />
    <title><?php echo $usrqr['fname']; ?> <?php echo $usrqr['lname']; ?> | Expiring Soon</title>
    <link
      rel="icon"
      type="image/png"
    ...
    $("#process").html("Saving...");
    $.post('admin-process.php', $(this).serialize(), function(data) {
    $('#process').html(data).css('color', '#00b207');
    });
    });
    ...
    $("#editaddress").on("keyup change", function(e) {
    $("#process2").html("Saving...");
    $.post('admin-process.php', $(this).serialize(), function(data) {
    $('#process2').html(data).css('color', '#00b207');
    });
    });
    });
...
var loadFile = function(event) {
  ...
  $("form#editPhoto").submit();
  });
  $("form#editPhoto").on("submit", function(event){
  event.preventDefault();
  var btn = document.getElementById("btn-avatar");
  btn.innerHTML = 'Uploading Avatar...';
  ...
  // KYC Status
  $(function() {
  $(".kycIcon").click(function(e){
  e.preventDefault();
  if(confirm("Update KYC Document Status?")){
  var uid = $(this).attr("id8");
  var status = $(this).attr("data-action");
  // $(this).hide();

  var info = 'id8=' + uid + '&docstatus=' + status;
   $.ajax({
     type: "GET",
     url: "process.php",
     data: info,
     success: function(){
     $("#kycstatus").html(status);
     // alert("KYC document status was updated");
     }
   });
  }
  else{
      ...
      <div class="breedcrumb__img-wrapper">
        ...
        <div class="container">
          <ul class="breedcrumb__content">
            ...
            <li class="active"><a href="#"><?php echo $usrqr['fname']; ?> <?php echo $usrqr['lname']; ?></a></li>
          </ul>
        </div>
      </div>
    </div>
        ...
        <div class="row dashboard__content">
          <div class="col-lg-3">
            <nav class="dashboard__nav">
              <h5 class="dashboard__nav-title font-body--xl-500">
                navigation
              </h5>
              <ul class="dashboard__nav-item">
                <li class="dashboard__nav-item-link">
                  <a href="admin-dashboard.php" class="font-body--lg-400">
                    <span class="icon">
                      <svg
                        width="24"
                        height="24"
                ...
                <li class="dashboard__nav-item-link active">
                  <a href="admin-users.php" class="font-body--lg-400">
                    <span class="icon">
                      <svg
                        width="20"
                        height="20"
                ...
                <!-- Manage Orders -->
                <li class="dashboard__nav-item-link">
                  <a href="admin-orders.php" class="font-body--lg-400">
                    <span class="icon">
                      <svg
                        width="16"
                        height="16"
                ...
                <?php if($aqr['level']=='Administrator'){ ?>
                <!-- Manage Products  -->
                <li class="dashboard__nav-item-link">
                  <a href="admin-products.php" class="font-body--lg-400">
                    <span class="icon">
                      <svg
                        width="24"
                        height="24"
                ...
                <!-- Payouts -->
                <li class="dashboard__nav-item-link">
                  <a href="admin-payouts.php" class="font-body--lg-400">
                    <span class="icon">
                      <svg
                        width="16"
                        height="16"
                ...
                <!--  Settings   -->
                <li class="dashboard__nav-item-link">
                  <a href="admin-settings.php" class="font-body--lg-400">
                    <span class="icon">
                      <svg
                        width="24"
                        height="24"
              ...
              <?php } ?>
                <!--  Log out    -->
                <li class="dashboard__nav-item-link">
                  <a href="admin-logout.php" class="font-body--lg-400">
                    <span class="icon">
                      <svg
                        width="24"
                        height="24"
            ...
            <div class="container">
              ...
              <div class="dashboard__content-card">
                <div class="dashboard__content-card-header">
                  <h5 class="font-body--xl-500">User ID #<?php echo $usrqr['id']; ?></h5>
                </div>
                <div class="dashboard__content-card-body">
                  <div class="row">
                    <div class="col-lg-7 order-lg-0 order-2">
                      <form method="post" id="editinfo">
                        <input type="hidden" name="uid" value="<?php echo $usrqr['id']; ?>">
                        <div class="contact-form__content">
                          ...
                          <div class="contact-form-input">
                            ...
                            <input
                              type="number"
                              name="phone"
                              value="<?php echo $usrqr['phone']; ?>"
                              placeholder"Phone Number"
                              onkeypress="validate(event)"
                            />
                          </div>
                          <div class="contact-form-input">
                            <label for="number1">Vendor Commission %</label>
                            <input
                              type="number"
                              name="commission"
                              value="<?php echo $usrqr['commission']; ?>"
                    ...
                    <div class="col-lg-5 order-lg-0 order-1">
                      <div class="dashboard__content-card-img">
                        <form method="post" enctype="multipart/form-data" style="text-align: center" id="editPhoto">
                          <input type="hidden" name="avatar" value="<?php echo $usrqr['id']; ?>">
                          <div class="dashboard__content-img-wrapper" id="avatar">
                            <img
                              src="<?php echo $usrqr['pic']; ?>"
                              alt="<?php echo $usrqr['fname']; ?> <?php echo $usrqr['lname']; ?>"
                               onclick="performClick('theFile');"
                               id="imgPreview"
                            />
                          </div>
                            <input type="file" name="theFile" id="theFile" onchange="loadFile(event)" accept=".png, .jpg, .jpeg" />
                          <button type="button" class="button button--outline" id="btn-avatar" onclick="performClick('theFile');">Upload Avatar/Logo</button>
                        </form>
                      </div>
                    </div>
                  </div>
              ...
              <div class="dashboard__content-card">
                <div class="dashboard__content-card-header">
                  <h5 class="font-body--xl-500">Delivery Address</h5>
                </div>
                <div class="dashboard__content-card-body">
                  <form method="post" id="editaddress">
                    <input type="hidden" name="uid2" value="<?php echo $usrqr['id']; ?>">
                    <div class="contact-form__content">
                      <div class="contact-form-input">
                      ...
                      <div class="contact-form__content-group">
                        ...
                        <div class="contact-form-input">
                          ...
                          >
                          <option value="<?php echo $usrqr['state']; ?>" selected><?php echo $usrqr['state']; ?></option>
              ...
              <div class="dashboard__content-card">
                ...
                <div class="dashboard__content-card-body">
                  <?php
                  $query = mysqli_query($con, "SELECT * FROM kyc WHERE userid='".$usrqr['id']."' ORDER BY date DESC");
                  ...
                  <div class="docimg"><a href="<?php echo $doc['document']; ?>" target="_blank"><img src="<?php echo $docimg; ?>"></a></div>
                  <div class="docinfo"><span style="font-size:14px"><?php echo $doc['idtype']; ?></span>
                  <br /><span style="font-weight:500;font-size:12px;text-transform:uppercase;color:<?php echo $statuscolor; ?>" id="kycstatus"><?php echo $doc['status']; ?></span>
                  </div>
                  <div class="docicon kycIcon" id8="<?php echo $doc['id']; ?>" data-action="Rejected">
                    <a href="#"><span class="iconify" style="color:#ff0000" data-icon="akar-icons:circle-x-fill" data-width="25" data-height="25"></a>
                    </div>
                    <div class="docicon kycIcon" id8="<?php echo $doc['id']; ?>" data-action="Approved">
                    <a href="#"><span class="iconify" style="color:#00b207" data-icon="akar-icons:check-box-fill" data-width="25" data-height="25"></a>
                    </div>
                  </div>
                  <br />
                <?php } ?>
              ...
              <div class="dashboard__content-card">
                ...
                <div class="dashboard__content-card-body">
                    ...
                    <?php
                    $query = "SELECT * FROM bankinfo WHERE userid='".$_GET['uid']."'";
                    $res = mysqli_query($con, $query);
                    while ($row = mysqli_fetch_array($res)){
                    echo ''.$row['bank'].'<br />'.$row['acctname'].'<br />
                    <span style="color:#000;font-weight:500">'.$row['acctno'].'</span>';
                    }
                    ?>
                  </div>
                </div>
              ...
              <div class="dashboard__content-card" id="requestPayout">
                <div class="dashboard__content-card-header">
                  <h5 class="font-body--xl-500">Payout Requests</h5>
                </div>
                <div class="dashboard__content-card-body">
                  <?php
                  echo '<div style="border-bottom:1px solid #f1f1f1;padding-bottom:10px;margin-bottom:10px">Balance: <span style="color:#00b207;font-weight:500">N'.number_format($uqr['wallet'], 2).'</span></br /></div>';

                  $query = "SELECT * FROM payouts WHERE userid='".$_GET['uid']."' ORDER BY date DESC";
                  $res = mysqli_query($con, $query);
                  echo '<span style="font-size:11px">Payout History</span>';
                  while ($row = mysqli_fetch_array($res)){
                  echo '<div style="border-bottom:1px solid #f1f1f1;padding-bottom:10px;margin-top:10px;margin-bottom:10px">
                  <span style="color:#00b207;font-weight:500">N'.number_format($row['amount'], 2).'</span> on '.date('d M, Y', strtotime($row['date'])).'<br /><span style="font-size:11px;text-transform:uppercase;font-weight:500">'.$row['status'].'</span></div>';
                  }
                 ?>
                </div>
              </div>
              ...
              <div class="dashboard__content-card">
                ...
                <div class="dashboard__content-card-body">
                    ...
                    <?php
                    $query = "SELECT * FROM discounts WHERE userid='".$_GET['uid']."' ORDER BY expiry ASC";
                    $res = mysqli_query($con, $query);
                    while ($row = mysqli_fetch_array($res)){
                    echo '<div style="border-bottom:1px solid #f1f1f1;padding-bottom:10px;margin-top:10px">'.$row['expiry'].' Days: <span style="color:#00b207;font-weight:500">'.$row['discount'].'% Discount</span></br /></div>';
                    } ?>
                  </div>
                </div>
              </div>
...
<!-- Mirrored from shopery.netlify.app/main/user-dashboard.html by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 29 Dec 2021 14:28:42 GMT -->
</html>
