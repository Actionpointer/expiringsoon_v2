...
<head>
    ...
    <title>Wishlist | Expiring Soon</title>
    <link
      rel="icon"
      type="image/png"
        ...
        <div class="shoping-cart__content">
          <div class="cart-table">
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                  <?php
                  // Fetch Products
                  $query = "SELECT * FROM likes WHERE userid='".$uqr['id']."' ORDER BY dateadded DESC LIMIT 50";
                  $proddata = mysqli_query($con,$query);
                  if (mysqli_num_rows($proddata) == 0) {
                  echo '<div style="margin:auto;padding:1%;text-align:center"><img style="padding:10px;width:100px" src="img/exclamation.png"><br />Your Wishlist is empty.<br /><a href="shop.php"><span style="font-size:13px;color:#00b207">Start Shopping Now!</span></a></div>';
                  }
                  while($row = mysqli_fetch_array($proddata)){

                  $pqq = mysqli_query($con, "SELECT * FROM products WHERE id='".$row['prod_id']."'");
                  ...
                  <tr class="likeditem">
                    ...
                    <td class="cart-table-item align-middle">
                      <a
                        href="product.php?pid=<?php echo $prqr['id']; ?>"
                      ...
                      >
                        ...
                        <h5 class="font-body--lg-400"><?php echo $prqr['product']; ?></h5>
                      </a>
                    </td>
                    <!-- Price  -->
                    <td class="cart-table-item order-date align-middle">
                      <?php if($prqr['expiry']!='' && $numberDays < 90) { ?>
                        <p class="font-body--lg-500">N<?php echo number_format($sale, 0); ?> <del>N<?php echo number_format($prqr['price'], 0); ?></del></p>
                      <?php } else{ ?>
                        <p class="font-body--lg-500">N<?php echo number_format($prqr['price'], 0); ?></p>
                      <?php } ?>
                    </td>
                    <!-- Stock Status  -->
                    <td class="cart-table-item stock-status align-middle">
                      <?php if($prqr['stock'] > 0 && $prqr['status']=='Listed'){ ?>
                    ...
                    <td class="cart-table-item add-cart align-middle">
                      <div class="add-cart__wrapper">
                        <?php if($prqr['stock'] > 0 && $prqr['status']=='Listed'){ ?>
                        <button class="button button--md add-to-cart" id3="<?php echo $prqr['id']; ?>" data-price="<?php echo $sale; ?>" data-product="<?php echo $prqr['product']; ?>" data-photo="<?php echo $prqr['photo']; ?>">Add to Cart</button>
                      <?php } else{ ?>
                        <button class="button button--md button--disable">Add to Cart</button>
                      <?php } ?>
                        <button class="delete-item" id2="<?php echo $prqr['id']; ?>">
                          <svg
                            width="24"
                            height="25"
                            viewBox="0 0 24 25"
            ...
            <div class="cart-table__share-content d-flex align-items-center">
              <span
                class="font-body--md-400"
                style="margin-right: 10px; color: #1a1a1a"
                >Share:
              </span>
          ...
          <div class="shoping-cart__mobile">
            ...
            <div class="shoping-card likeditem">
              ...
              <h6 class="shoping-card__product-price font-body--lg-600">
                N<?php echo number_format($sale, 0); ?> <del class="prev-price">N<?php echo number_format($prqr['price'], 0); ?>
              </h6>
            ...
            <?php } else{ ?>
              <button class="button button--md button--disable">Add to Cart</button>
            <?php } ?>
              <button class="close-btn delete-item" id2="<?php echo $prqr['id']; ?>">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
