...
$sum = "SELECT sum(amount) as ttpaid FROM payouts WHERE userid='".$uqr['id']."' AND status='Paid'";
$res = mysqli_query($con, $sum);
while ($row = mysqli_fetch_array($res)){
$ttlpayout = $row['ttpaid'];
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Payput History | Expiring Soon</title>
    <link
      rel="icon"
      type="image/png"
            ...
            <div class="container">
              ...
              <div class="dashboard__order-history">
                <div class="dashboard__order-history-title">
                  <h2 class="font-body--xxl-500">Payout History</h2>
                  <a href="#" class="font-body--lg-500">
                    N<?php echo number_format($ttlpayout, 0); ?> Total</a>
                </div>
                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <tbody>
                        <?php
                        // Fetch Orders
                        $query = "SELECT * FROM payouts WHERE userid='".$uqr['id']."' ORDER BY date DESC";
                        $paydata = mysqli_query($con,$query);
                        if (mysqli_num_rows($paydata) == 0) {
                        echo '<div style="margin:auto;padding:1%;text-align:center;margin-bottom:5%"><img style="padding:10px;width:100px" src="img/exclamation.png"><br />No Payout Requests at this time.</div>';
                        }
                        while($row = mysqli_fetch_array($paydata)){
                        ...
                        <tr>
                          ...
                          <p align="left" style="color:#d92e2e;font-size:14px"><?php echo $row['status']; ?></p>
                        <?php } else { ?>
                          <p align="left" style="color:#000;font-size:14px;font-weight:500"><?php echo $row['status']; ?></p>
                        <?php } ?>
                          </td>
