...
$sum = "SELECT sum(amount) as ttl FROM payouts WHERE status='Pending'";
...
<head>
    ...
    <title>Manage Payouts | Expiring Soon</title>
    <link
      rel="icon"
      type="image/png"
        ...
        <div class="row dashboard__content">
          ...
          <div class="col-lg-9 section--xl pt-0" style="padding:10px;font-size:13px">
            <div class="dashboard__order-history-title" style="margin:auto;width:95%;border-bottom:1px solid #ddd;margin-bottom:10px">
              ...
              <a href="#" class="font-body--lg-500">N<?php echo number_format($ttlpayouts, 2); ?> Total</a>
            </div>
            <div class="container">
              ...
              <table id="datatable" class="table display" style="width:100%;font-size:13px">
                  ...
                  <tbody>
                    <?php
                    // Fetch Payouts
                    $query = "SELECT * FROM payouts ORDER BY date DESC LIMIT 50";
                    $userdata = mysqli_query($con,$query);
                    if (mysqli_num_rows($userdata) == 0) {
                    echo '<div style="margin:auto;padding:1%;text-align:center"><img style="padding:10px;width:100px" src="img/exclamation.png"><br />No Payouts Requests at this time.</span></div>';
                    }
                    while($row = mysqli_fetch_array($userdata)){
                      $uqq = mysqli_query($con, "SELECT * FROM users WHERE id='".$row['userid']."'");
                      $uqr = mysqli_fetch_assoc($uqq);

                      $bkq = mysqli_query($con, "SELECT * FROM bankinfo WHERE userid='".$row['userid']."'");
                      $bkr = mysqli_fetch_assoc($bkq);
                    ?>
                    <tr class="likeditem" style="border-bottom:1px solid #f1f1f1">
                      <!-- item  -->
                      <td class="cart-table-item order-date align-middle">
                      <div style="margin-top:10px"><span class="font-body--lg-500" style="color:#000"><?php echo $uqr['fname']; ?> <?php echo $uqr['lname']; ?></span>
                        <br />
                        <span style="font-size:12px;color:#888"><?php echo $bkr['bank']; ?><br /><?php echo $bkr['acctname']; ?> > <span style="color:#000"><?php echo $bkr['acctno']; ?></span></span>
                      </div>
                      </td>
                      <!-- Price  -->
                      <td class="cart-table-item order-date align-middle">
                        <p class="font-body--lg-500" style="color:#00b207">N<?php echo number_format($row['amount'], 2); ?></p>
                      </td>
                      <!-- Stock Status  -->
                      <td class="cart-table-item order-date align-middle">
