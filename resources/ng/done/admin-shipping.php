...
if($aqr['level']!=='Administrator'){
header("Location: admin-dashboard.php");
}

$sum = "SELECT sum(amount) as tt_shipping FROM shipping WHERE status='Pending'";
$res = mysqli_query($con, $sum);
while ($row = mysqli_fetch_array($res)){
$ttlshipping = $row['tt_shipping'];
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Manage Shipping | Expiring Soon</title>
    <link
      rel="icon"
      type="image/png"
    ...
    <script src="datatable/assets/buttons/demo.css"></script>
    <script src="datatable/assets/buttons/demo.js"></script>
    <script src="datatable/assets/buttons/dataTables.buttons.min.js"></script>
    <script src="datatable/assets/buttons/jszip.min.js"></script>
    <script src="datatable/assets/buttons/pdfmake.min.js"></script>
    <script src="datatable/assets/buttons/vfs_fonts.js"></script>
    <script src="datatable/assets/buttons/buttons.html5.min.js"></script>
    <script src="datatable/assets/buttons/buttons.print.min.js"></script>
    ...
    e.preventDefault();
    if(confirm("Item will be marked as Paid. Are you sure?")){
    var element = $(this);
    var pid = element.attr("id6");
    $(this).hide();
    ...
    e.preventDefault();
    if(confirm("All Pending payments will be marked as Paid. Are you sure?")){
    ...
    var info = 'id7=' + action;
     $.ajax({
       ...
       success: function(){
       $("#status").html("Paid").css({'color':'#00b207', 'font-weight':'500'});
       window.location = "admin-shipping.php";
       }
     });
   }
   else{
        ...
        <div class="row dashboard__content">
          ...
          <div class="col-lg-9 section--xl pt-0" style="padding:10px;font-size:13px">
            <div class="dashboard__order-history-title" style="margin:auto;width:95%;border-bottom:1px solid #ddd;margin-bottom:10px">
              ...
              <a href="#" class="font-body--lg-500">N<?php echo number_format($ttlshipping, 2); ?> Total</a>
            </div>
            <div class="container">
              <div style="margin-bottom:10px;border-bottom:1px solid #ddd;padding-bottom:10px"><a href="admin-payouts.php" style="font-size:13px;color:#00b207;font-weight:500">Payout Requests</a> | <a href="admin-shipping.php" style="font-size:13px;color:#00b207;font-weight:500">Shipping Settlements</a></div>
              <!-- Products -->
              <table id="datatable" class="table display" style="width:100%;font-size:13px">
                  ...
                  <tbody>
                    <?php
                    // Fetch Payouts
                    $query = "SELECT * FROM shipping ORDER BY date DESC LIMIT 50";
                    $userdata = mysqli_query($con,$query);
                    if (mysqli_num_rows($userdata) == 0) {
                    echo '<div style="margin:auto;padding:1%;text-align:center"><img style="padding:10px;width:100px" src="img/exclamation.png"><br />No Shipping History at this time.</span></div>';
                    }
                    while($row = mysqli_fetch_array($userdata)){
                      $uqq = mysqli_query($con, "SELECT * FROM users WHERE id='".$row['vendor']."'");
                      $uqr = mysqli_fetch_assoc($uqq);
                    ?>
                    <tr class="likeditem" style="border-bottom:1px solid #f1f1f1">
                      <!-- item  -->
                      <td class="cart-table-item order-date align-middle">
                      <div style="margin-top:10px"><span class="font-body--lg-500"><a href="admin-invoice.php?ref=<?php echo $row['orderid']; ?>" style="color:#00b207">#<?php echo $row['orderid']; ?></a></span></div>
                      </td>
                      <!-- Price  -->
                      <td class="cart-table-item order-date align-middle">
                        <p class="font-body--lg-500" style="color:#000">N<?php echo number_format($row['amount'], 2); ?></p>
                      </td>
                      <!-- Stock Status  -->
                      <td class="cart-table-item order-date align-middle">
                        <span style="font-size:12px;color:#888"><?php echo date('l, F d, Y', strtotime($row['date'])); ?></span>
                        <?php if($row['status']=='Pending'){ ?>
                        <p style="color:#d92e2e;font-size:14px"><span id="status"><?php echo $row['status']; ?></span></p>
                      <?php } else { ?>
                        <p style="color:#00b207;font-size:14px;font-weight:500"><?php echo $row['status']; ?></p>
                      <?php } ?>
                      </td>
                      <td class="cart-table-item add-cart align-middle">
                        <div class="add-cart__wrapper">
                          <a
                            href="#"
                            class="cart-table__product-item approve"
                            id6="<?php echo $row['id']; ?>"
                          >
                          <?php if($row['status']=='Pending'){ ?>
