...
$query = "SELECT * FROM products WHERE expiry!=''";
$proddata = mysqli_query($con,$query);
while($row = mysqli_fetch_array($proddata)){
if(strtotime("today") > strtotime($row['expiry'])){
$con->autocommit(FALSE);
$con->query("UPDATE products SET status='Expired' WHERE id='".$row['id']."'");
$con->commit();
}
}

$sum = "SELECT count(id) as ttprod FROM products";
$res = mysqli_query($con, $sum);
while ($row = mysqli_fetch_array($res)){
$ttl_products = $row['ttprod'];
}

$sum = "SELECT count(id) as ttordr FROM orders";
$res = mysqli_query($con, $sum);
while ($row = mysqli_fetch_array($res)){
$ttl_orders = $row['ttordr'];
}

$sum = "SELECT sum(total) as ttl FROM cart";
$res = mysqli_query($con, $sum);
while ($row = mysqli_fetch_array($res)){
// $ttl_sales = (80 / 100) * $row['ttl'];
$ttl_sales = $row['ttl'];
}
$ttl_payout = 0.00;
?>
...
<head>
    ...
    <title>Admin Dashboard | Expiring Soon</title>
    <link
      rel="icon"
      type="image/png"
                ...
                <div class="col-lg-7">
                  <div class="dashboard__user-profile dashboard-card">
                    <div class="dashboard__user-profile-img">
                      <img src="img/avatar.png" alt="<?php echo $aqr['fname']; ?>" />
                    </div>
                    <div class="dashboard__user-profile-info">
                      <h5 class="font-body--xxl-500 name">Hi, <?php echo $aqr['fname']; ?></h5>
                      <p class="font-body--md-400 designation"><?php echo $aqr['level']; ?></span></p>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="dashboard__user-billing dashboard-card">
                    ...
                    <div class="dashboard__user-billing-info">
                      ...
                      <h5
                        class="dashboard__user-billing-name font-body--xl-500"
                      >
                        N<?php echo number_format($ttl_sales, 0); ?>
                      </h5>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <tbody>
                        <?php
                        // Fetch Orders
                        $kycdata = mysqli_query($con, "SELECT * FROM kyc WHERE status='Pending' ORDER BY date DESC LIMIT 20");
                        if (mysqli_num_rows($kycdata) == 0) {
                        echo '<div style="margin:auto;padding:1%;text-align:center;margin-bottom:5%"><img style="padding:10px;width:100px" src="img/exclamation.png"><br />No pending documents at this time</div>';
                        }
                        while($row = mysqli_fetch_array($kycdata)){
                        $usrq = mysqli_query($con, "SELECT * FROM users WHERE id='".$row['userid']."'");
                        ...
                        <tr>
                          ...
                          >
                          <img src="<?php echo $usrqr['pic']; ?>" alt="Pic" style="width:50px;border-radius:50px;border:1px solid #ddd;padding:3px" />
                          </td>
                          <!-- Date  -->
                          <td
                            class="
                          ...
                          <p class="order-total-price">
                            <?php echo $usrqr['fname']; ?> <?php echo $usrqr['lname']; ?><br />
                            <span style="font-size:12px;color:#888"><?php echo $usrqr['email']; ?></span>
                          </p>
                          </td>
                          <!-- Total  -->
                          <td
                          ...
                          <td
                            ...
                            "
                          >
                            <span style="color:#ff0000"><?php echo $row['status']; ?></a>
                          </td>
                          <!-- Details page  -->
                          <td
                            class="
                            ...
                            "
                          >
                            <a href="admin-edituser.php?uid=<?php echo $usrqr['id']; ?>"> View User</a>
                          </td>
                        </tr>
                      <?php } ?>
                      </tbody>
                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <tbody>
                        <?php
                        // Fetch Orders
                        $query = "SELECT * FROM orders ORDER BY date DESC LIMIT 20";
                        $proddata = mysqli_query($con,$query);
                        ...
                        <tr>
                          ...
                          >
                            <p class="order-total-price">
                              N<?php echo number_format($finalttl, 0); ?>
                            </p>
                          </td>
                          <!-- Status -->
                          <td
                          ...
                          <td
                            ...
                            "
                          >
                            <a href="admin-invoice.php?ref=<?php echo $row['orderid']; ?>"> View Details</a>
                          </td>
                        </tr>
                      <?php } ?>
                      </tbody>
