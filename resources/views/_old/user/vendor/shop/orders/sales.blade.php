                ...
                <div class="table-responsive">
                  <table id="datatable" class="table display" style="width:100%;font-size:13px">
                    ...
                    <tbody>                     
                        @forelse($shop->carts->where('status','Delivered')->sortByDesc('date')->take(10) as $cart)
                                ...
                                <tr>
                                    ...
                                    <td class="dashboard__order-history-table-item order-total "> 
                                        <p class="order-total-price">   {!!$shop->country->currency->symbol!!}{{number_format($shop->commission / 100 * $cart->total, 0)}} </p>
                                    </td>
                                    <!-- Site Split  -->
                                    <td class="dashboard__order-history-table-item order-total"> 
                                        <p class="order-total-price">   {!!$shop->country->currency->symbol!!}{{number_format($cart->total - ($shop->commission / 100 * $cart->total), 0)}} </p>
                                    </td>
                                    <!-- Status -->
                        ...
                        @empty
                            <div style="margin:auto;padding:1%;text-align:center;margin-bottom:5%"><img style="padding:10px;width:100px" src="{{asset('images/site/exclamation.png')}}"><br />You have no orders at this time.</div>
                        @endforelse
                    </tbody>
                  </table>
                </div>
