                                            ...
                                            <div class="cards-md cards-md--four w-100">
                                                <div class="cards-md__img-wrapper">
                                                    <a href="{{route('product.show',$product)}}">
                                                        <img src="{{Storage::url($product->photo)}}" alt="{{$product->name}}" onerror=this.src="{{asset('images/site/no-image.png')}}"; />
                                                    </a>
...
@push('scripts')
    ...
    <script>
        $('.select2').select2()
        //when its showing following, class is unfollow, text to change to unfollow
        $(document).on('mouseover','.unfollow',function(){
            $(this).removeClass("bg-white text-success");
            $(this).css("background-color", '#e20c0c');
            $(this).text('Unfollow')
        })
        //when its
        $(document).on('mouseout','.unfollow',function(){
            $(this).addClass("bg-white text-success");
            $(this).text('Following')
        })
