                    ...
                    <div class="d-flex w-50">
                      <div class="user-img">
                        <img class="rounded-circle" alt="user-photo"
                            @if($message->admin_id)
                                @if(!$message->admin->pic) src="{{asset('img/avatar.png')}}" @else src="{{Storage::url($message->admin->pic)}}" @endif 
                            @else 
                                @if(!$message->user->pic) src="{{asset('img/avatar.png')}}" @else src="{{Storage::url($message->user->pic)}}" @endif 
                            @endif
                        >
                      </div>
                      ...
                      <div class="user-message-info">
                          <div class="d-flex ">
                            <h5 class="font-body--md-500">@if($message->admin_id) {{$message->admin->name}} @else {{$message->user->name}}  @endif</h5>
