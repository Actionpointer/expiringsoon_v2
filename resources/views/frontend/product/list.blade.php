                          ...
                          <select id="category" class="filter__dropdown-menu" onChange="filterProducts()">
                            <option value="All">All Categories</option>
                                @foreach ($categories->sortBy('category') as $cat)
                                    <option value="{{$cat->id}}" @if($category->id == $cat->id) selected @endif>{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="select-box--item">
                        ...
                        <div class="active__filters">
                            ...
                            <div class="active__filters-item">
                                <button class="filter-item">
                                    <span id="active2">{{$category->name}}</span>
                                    <span class="icon">
                                    ...
                                    <div class="cards-md cards-md--four w-100">
                                        <div class="cards-md__img-wrapper">
                                            <a href="{{route('product.show',$product)}}">
                                                <img src="./storage/{{$product->photo}}" alt="{{$product->name}}" onerror="this.src='img/no-image.png';" />
                                            </a>
                                            @if($product->expiry !='' && $product->expiry->diffInDays(now()) < 90 && $product->stock > 0)
                                        ...
                                        <div class="cards-md__info d-flex justify-content-between align-items-center">
                                            <a href="{{route('product.show',$product)}}" class="cards-md__info-left">
                                                ...
                                                <div class="cards-md__info-price">
                                                    @if($product->expiry !='' && $product->expiry->diffInDays(now()) < 90)
                                                        <span class="font-body--lg-500">N{{number_format(( $product->price - ($product->price * $discount/100)), 0)}}</span>
                                                        <del class="font-body--lg-400" style="color:#00b207">N{{number_format(($product->price), 0)}}</del>
                                                    @else
                                                        <span class="font-body--lg-500">N {{number_format($product->price, 0)}}</span>
                                                    @endif
                                                </div>
                                                <ul class="d-flex" style="color:#888;font-size:12px">
                                                    @if($product->expiry !='' && $product->expiry->diffInDays(now()) < 90)
                                                        <li>Expires in <span style="font-weight:550;color:#00b207">{{$product->expiry->diffInDays(now())}} days</span><li>
                                                    @else
                                                    <li>&nbsp;</li>
                                                    @endif
                                                </ul>
                                            ...
                                            <div class="cards-md__info-right">
                                                @if($product->stock > 0)
                                                    <span class="action-btn">
                                                        @if($product->expiry !='' && $product->expiry->diffInDays(now()) < 90)
                                                            <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg" class="add-to-cart" id3="{{$product->id}}" data-price="{{$sale}}" data-product="{{$product->id}}product" data-photo="./storage/{{$product->photo}}">
                                                        @else
                                                            <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg" class="add-to-cart" id3="{{$product->id}}" data-price="{{$product->price}}" data-product="{{$product->id}}product" data-photo="./storage/{{$product->photo}}">
                                                        @endif
                                                                <path
