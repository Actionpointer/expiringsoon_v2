													...
													<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                        <tr>
															<td class="h1 pb25" style="color:#666; font-family:Poppins,sans-serif; font-size:13px; line-height:25px; text-align:center; padding-bottom:15px;">
																<span style="font-size:16px;font-weight:600">Subscription is {{$status}}!</span><br />
														...
														<tr>
															<td class="h1 pb25" style="color:#666; font-family:Poppins,sans-serif; font-size:13px; line-height:25px; text-align:left; padding-bottom:15px;">
                                                                ...
                                                                @if($status == 'activated') has been activated. <br>
																	...
																	Products: {{$subscription->user->products->count()}} <br/>
																	Shops: {{$subscription->user->shops->count()}}
                                                                @elseif($status == 'expiring') is about to expire. <br>
                                                                    Subscription will expire on {{$subscription->end_at->format('d-m-y h:i A')}}

                                                                @else has expired. <br>
                                                                    You have been automatically subscribed to Free Plan so you may continue to use expiring soon with limited features
                                                                @endif
														...
														<tr>
															<td class="text-btn-large" align="center">
																<div style="background:#5fb882;width:50%;color:#fff; font-family:'Poppins', Arial,sans-serif; font-size:15px; line-height:18px; text-align:center;margin-top:20px; padding:15px 35px;border-bottom:1px solid #ddd">
																	@if($status != 'activated')
																	<a href="{{route('vendor.dashboard')}}" target="_blank" class="link-2" style="color:#fff; text-decoration:none;">
																		<span class="link-2" style="color:#fff; text-decoration:none;">Renew Subscription</span>
																	</a>
                                                                    @else
                                                                    <a href="{{route('vendor.plans')}}" target="_blank" class="link-2" style="color:#fff; text-decoration:none;">
																		<span class="link-2" style="color:#fff; text-decoration:none;">View Subscription Details</span>
																	</a>
																	@endif
																</div>
															</td>
