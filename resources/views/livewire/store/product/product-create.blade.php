                ...
                <div class="card mb-6 card-lg">
                    ...
                    <div class="card-body p-6">
                        ...
                        <div class="form-check form-switch mb-4">
                            <input class="form-check-input" type="checkbox" role="switch" wire:model="always_available" id="flexSwitchStock"  />
                            <label class="form-check-label" for="flexSwitchStock">Always Available</label>
                        </div>
                        <div class="form-check form-switch mb-4">
                            <input class="form-check-input" type="checkbox" role="switch" wire:model="preorder" id="flexSwitchStock"  />
...
@push('scripts')
    ...
    <script>
        // Wait for both jQuery and Livewire to be available
        function waitForDependencies() {
            if (typeof $ === 'function' && typeof Livewire !== 'undefined') {
                initializeSelect2();
            } else {
        ...
        $(document).ready(function(){
            ...
            function updateVariantIndices() {
                $('.variant-item').each(function(index) {
                   ...
                   // Force Livewire to update the name
                   console.log('dispatching', productName);
                    Livewire.dispatch('variantNameUpdated', {
                        index: index,
                        name: productName
                    });
            ...
            function updateVariantsBasedOnAttributes() {
                const productName = $('#productName').val(); // Get current product name
                $('.variant-item').each(function(index) {
            ...
            console.log('Generating variant fields with selectedAttributes:', selectedAttributes);
                ...
                //  Inject name from product name input
                const productName = $('#productName').val() || 'Unnamed Variant';

                // Add hidden name input
                const nameFieldHtml = `
                    <input type="hidden" class="variant-name" wire:model="variants.${variantIndex}.name" value="${productName}">
                `;

                optionsContainer.append(nameFieldHtml);
                ...
                // Re-trigger Livewire input sync (important for hidden fields)
                variantElement.find('.variant-name').val(productName).trigger('input');
            }
            ...
            $('#lfm').filemanager('image', {prefix: route_prefix,
                getFileCallback: function(fileUrl) {
                    ...
                    const properUrl = fileUrl
                        .replace(/^.*\/storage\//, '/storage/')  // Remove duplicate paths
                        // .replace('localhost', window.location.hostname);  // Remove duplicate slashes
                    ...
                    // Update preview immediately (using asset() helper)
                    $('#holder').html(`
                        <img src="/${properUrl}" 
            ...
            function updateImagePreview(url) {
                if (url) {
                    // Create proper asset URL
                    const previewUrl = url.startsWith('http') ? url : 
        ...
        // Start the process
        document.addEventListener('livewire:load', waitForDependencies);
