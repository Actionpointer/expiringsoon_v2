...
trait PayoutTrait
{
    protected function initializePayout(Payout $payout){
        ...
        switch($gateway){
            case 'paystack': 
                $link = $this->initiatePaystack($payout->amount,'currency','bank','account_no');
                // $user = auth()->user();
                // $currency = Setting::firstWhere('name','currency')->value;
                ...
                // dd($response);
            break;
            case 'flutter': $link = $this->initiateFlutterWave($payout->amount,'currency','bank','account_no');
            break;
    ...
    protected function initiateFlutterWave($amount,$currency,$bank,$account_number){
        $user = Auth::user();
    ...
    public function initiatePaystack($amount,$currency,$bank,$account_number){
      $user = auth()->user();
      $currency = Setting::firstWhere('name','currency')->value;
      $response = Curl::to('https://api.paystack.co/transaction/initialize')
      ->withHeader('Authorization: Bearer '.config('services.paystack.secret'))
